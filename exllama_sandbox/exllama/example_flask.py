...
model_directory = "./TableModel-GPTQ4bit"

tokenizer_path = os.path.join(model_directory)
...
@app.route('/infer_bench', methods=['POST'])
def inferContextB():
    # print(request.json)
    ...
    generator.settings.beams = settings.get('beams')
    generator.settings.beam_length = settings.get('beam_length')
    generator.in_beam_search = settings.get('in_beam_search')
    

    outputs = generator.generate_simple(prompt, max_new_tokens = settings.get('max_new_tokens'))
    return outputs
